...
def test_connect():
    ...
    fetch_messages = "select text, username from messages join users on users.id = messages.user_id"
...
def on_search(search):
    ...
    conn = connectToDB()
    cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
    search_query = "select username, text from messages join users on messages.user_id = users.id where text like %s or username like %s"
...
def on_login(data):
    ...
    print(cur.mogrify(login_query, (data['username'], data['password'])))
